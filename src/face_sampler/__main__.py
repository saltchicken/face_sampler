...
def run_esrgan(input, output, scale=2, resolution=None):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        executable_path = os.path.join(
            current_directory, "esrgan/realesrgan-ncnn-vulkan.exe"
        )
...
def main():
    ...
    parser.add_argument(
        "-r", "--rate", default="10", type=str, help="Framerate for video conversion"
    )
    parser.add_argument(
        "--video", action="store_true", help="Converting video to faces"
    )
    parser.add_argument(
